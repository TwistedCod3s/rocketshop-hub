      ...
      <div className="relative" ref={containerRef}>
        ...
        <div className="absolute top-1/2 -left-4 z-10 -translate-y-1/2">
          <Button
            variant="outline"
            size="icon"
            className="rounded-full bg-white border-gray-200 shadow-md hover:bg-white hover:border-gray-300"
            onClick={prev}
          >
          ...
          {Array.from({ length: Math.ceil(products.length / itemsPerView) }).map((_, i) => (
            <button
              ...
              className={`h-2 rounded-full transition-all ${
                i === Math.floor(current / itemsPerView)
                  ? "w-6 bg-rocketry-navy"
                  : "w-2 bg-gray-300"
              }`}
              onClick={() => setCurrent(i * itemsPerView)}
            />
          ))}
        </div>
      </div>
